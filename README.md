...
---
#### `<article>`
HTML5 中引入的一個語意化元素，專門用來表示獨立的內容單元，如博客文章、新聞報導、評論、論壇帖子等。這些內容單元通常具有自成一體的意義，並且可以在上下文中單獨存在。

##### 主要特點

1. **語意化**：
   - `<article>` 是語意化的 HTML 元素，這意味著它不僅僅是用來佈局或包裹內容的，還具有描述性，告訴瀏覽器和搜索引擎該區塊的內容是獨立的可復用單位。

2. **可獨立存在**：
   - 內容可以獨立於頁面或網站的其他部分而存在。例如，RSS 提要中的每個項目通常是一篇獨立的文章，可以用 `<article>` 來包裹。

3. **嵌套**：
   - 可以嵌套使用 `<article>` 元素。例如，一篇博客文章的主體內容可以包裹在 `<article>` 中，而其內部的評論部分也可以用 `<article>` 來包裹，表示這些評論也是獨立的內容單元。
...
##### 使用場景
- **獨立內容**：例如新聞網站的每一篇報導、博客中的每一篇文章、產品評論等都應使用 `<article>` 來包裹。
- **內容聚合**：在新聞聚合器或其他聚合平台上，`<article>` 元素可以用來標記各種來源的文章內容。

使用 `<article>` 元素可以幫助改善網頁的結構化數據，使內容更容易被搜索引擎理解和索引，從而有助於 SEO 優化。

---
#### `<section>` 
HTML5 中的一個語意化元素，用來表示文檔中的一個區塊或部分，這個區塊通常有其特定的主題或內容。`<section>` 常用於組織和劃分頁面內容，使得文檔結構更加清晰。

##### 特點

1. **語意化元素**：
   - `<section>` 是一個語意化元素，意在表示文檔中的一個邏輯區塊或部分，這個部分通常圍繞著一個共同的主題或內容。

2. **包含標題**：
   - 通常，`<section>` 中應包含標題 (`<h1>` 至 `<h6>`) 來描述該區塊的主題。這有助於結構化內容，並提高可讀性和 SEO（搜尋引擎優化）。
...
- **與 `<article>` 的區別**：
  - `<article>` 用於表示一個獨立的、自包含的內容單元，可以單獨存在或被引用，如一篇博客文章或新聞報導。
  - `<section>` 更適合用來劃分文檔中的不同主題部分，而這些部分通常是文檔的一部分而非獨立內容。
...
##### 使用場景
- **多部分的文章或內容**：將長文章分成多個部分，每個部分使用 `<section>` 來劃分主題。
- **頁面主要區塊**：用來定義網頁中的主要部分，如標頭、主內容區、頁尾等。

使用 `<section>` 元素可以幫助你組織頁面結構，使其更加清晰易讀，並且更容易被搜索引擎理解和索引。
...
1. **語意化元素**：
   - `<header>` 是語意化的元素，表示文檔或區段的開頭部分，通常包含主要的導航和介紹性內容。
...
使用 `<header>` 元素能夠幫助提高網頁結構的清晰度，使得內容更容易被搜索引擎和輔助技術理解，從而提升網站的可訪問性和 SEO 效果。

---
#### `<footer>` 
HTML5 中的一個語意化元素，用來表示文檔或區塊的底部內容。通常包括有關頁面或區塊的附加信息，例如版權信息、聯絡方式、社交媒體連結、隱私政策、以及其他相關的輔助性內容。
...
使用 `<footer>` 能夠有效地組織頁面內容，使頁面結構更加清晰，並有助於提升網頁的可讀性和搜索引擎優化效果。
...
2. **專門用於導航**：
   - 該元素專門用來包裹一組導航鏈接，而不是普通的超鏈接組合。它幫助搜索引擎和輔助技術（如螢幕閱讀器）識別和定位網站的主要導航部分。
...
- **頁內目錄**：用於長文章或文檔的內部導航，使用戶可以快速跳轉到頁面內的特定部分。
...
- **唯一性**：每個頁面應該只包含一個 `<main>` 元素，這有助於輔助技術和搜索引擎更好地理解頁面結構。
...
3. **靈活的定位**：
   - `<aside>` 可以放置在文檔的任何部分，無論是與主內容並排，還是在文章內部，以提供補充信息。
